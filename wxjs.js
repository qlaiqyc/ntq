          ...
          if (typeof msgWrap[_EVENT_ID] === 'string' ) {
               if (typeof _event_hook_map_for3rd[msgWrap[_EVENT_ID]] === 'function' && _isIn3rdApiList(msgWrap[_EVENT_ID])) {
                   ...
                   if (shareEvents.indexOf(msgWrap[_EVENT_ID]) > -1) { // 判断是否分享事件，这里漏了要加上
                       // 自定义分享内容，申请分享权限
                       _sendMessage(JSON.stringify({'__handleFromWeixin':_xxyy, '__fullApiName':msgWrap['__params']['fullApiName']}));
                   }
    ...
    function _setDefaultEventHandlers() {
      ...
      _on('menu:share:timeline',function(argv){
        ...
        if (typeof argv.title === 'string') {
          data = argv;
          // 申请临时分享权限
          _sendMessage(JSON.stringify({'__handleFromWeixin':_xxyy, '__fullApiName': 'shareTimeline'}));
      ...
      _on('menu:share:qq', function(argv){
        ...
        if (typeof argv.title === 'string') {
          data = argv;
          // 申请临时分享权限
          _sendMessage(JSON.stringify({'__handleFromWeixin':_xxyy, '__fullApiName': 'shareQQ'}));
          _call('shareQQ',data);
        }else{
          data = {
              "link": document.documentURI || _session_data.init_url,
      ...
      _on('menu:share:weiboApp',function(argv){
        ...
        if (typeof argv.content === 'string') {
          ...
          var shareFunc = function(_img){          
            ...
            // 申请临时分享权限
            _sendMessage(JSON.stringify({'__handleFromWeixin':_xxyy, '__fullApiName': 'shareWeiboApp'}));
      ...
      _on('menu:share:QZone', function(argv){
        ...
        if (typeof argv.title === 'string') {
          data = argv;
          // 申请临时分享权限
          _sendMessage(JSON.stringify({'__handleFromWeixin':_xxyy, '__fullApiName': 'shareQZone'}));
	  ...
	  _on('menu:share:email',function(argv) {
        ...
        if (typeof argv.title === 'string') {
          data = argv;
          // 申请临时权限
          _sendMessage(JSON.stringify({'__handleFromWeixin':_xxyy, '__fullApiName': 'sendEmail'}));
